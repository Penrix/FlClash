...
class VpnService : SystemVpnService(), ManagedService, VpnHealthSignalSink {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        // through this callback instead of the bound-service path. Re-enter the app state
    ...
    private fun acquireKeepAliveWakeLock() {
        ...
        keepAliveWakeLock = powerManager.newWakeLock(
            ...
            "$packageName:VpnKeepAlive",
        ).apply {
            setReferenceCounted(false)
            acquire()
        }
        GlobalState.log("VPN keepalive wakelock acquired")
    }

    private fun releaseKeepAliveWakeLock() {
        val wakeLock = keepAliveWakeLock ?: return
        keepAliveWakeLock = null
        if (wakeLock.isHeld) {
            runCatching { wakeLock.release() }
        }
        GlobalState.log("VPN keepalive wakelock released")
    }
